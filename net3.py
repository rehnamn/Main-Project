...
# Fit the model
net3.fit(feature_train, label_train, validation_data=(feature_test, label_test), epochs=10, batch_size=200, verbose=2)
# Final evaluation of the model
loss,acc = net3.evaluate(feature_test, label_test, verbose=2)
print("CNN Accuracy: " ,acc*100)

    
# serialize model to JSON
model_json = net3.to_json()
with open("net3.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
net3.save_weights("net3.h5")

with open("acc_net3.json", "w") as json_file:
