...
for i in range(0,len(image)):
    try:
        ...
        resized_image=cv2.resize(full_size_image, (17,17), interpolation=cv2.INTER_CUBIC)
...
feature = feature.reshape(t,17,17,1).astype('float32')
...
def shallow_cnn1():
    ...
    model1 = Sequential()
    model1.add(Conv2D(16, (3,3),input_shape=(17,17, 1), activation='relu'))
    model1.add(Conv2D(36, (7,7), activation='relu'))
...
# Fit the model
net0.fit(feature_train, label_train, validation_data=(feature_test, label_test), epochs=10, batch_size=200, verbose=2)
# Final evaluation of the model
loss,acc = net0.evaluate(feature_test, label_test, verbose=2)
print("CNN Accuracy: " ,acc*100)

    
# serialize model to JSON
model_json = net0.to_json()
with open("net0.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
net0.save_weights("net0.h5")

with open("acc_net0.json", "w") as json_file:
