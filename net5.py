...
for i in range(0,len(image)):
    try:
        ...
        resized_image=cv2.resize(full_size_image, (13,13), interpolation=cv2.INTER_CUBIC)
...
feature = feature.reshape(t,13,13,1).astype('float32')
...
def shallow_cnn1():
    ...
    model1 = Sequential()
    model1.add(Conv2D(16, (3,3),input_shape=(13,13, 1), activation='relu'))
...
# Fit the model
net5.fit(feature_train, label_train, validation_data=(feature_test, label_test), epochs=10, batch_size=200, verbose=2)
# Final evaluation of the model
loss,acc = net5.evaluate(feature_test, label_test, verbose=2)
print("CNN Accuracy: " ,acc*100)

    
# serialize model to JSON
model_json = net5.to_json()
with open("net5.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
net5.save_weights("net5.h5")

with open("acc_net5.json", "w") as json_file:
