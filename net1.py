...
for i in range(0,len(image)):
    try:
        ...
        resized_image=cv2.resize(full_size_image, (11,11), interpolation=cv2.INTER_CUBIC)
...
feature = feature.reshape(t,11,11,1).astype('float32')
...
def shallow_cnn1():
    ...
    model1 = Sequential()
    model1.add(Conv2D(16, (3,3),input_shape=(11,11, 1), activation='relu'))
    model1.add(Conv2D(36, (5,5), activation='relu'))
...
# Fit the model
net1.fit(feature_train, label_train, validation_data=(feature_test, label_test), epochs=10, batch_size=200, verbose=2)
# Final evaluation of the model
loss,acc = net1.evaluate(feature_test, label_test, verbose=2)
print("CNN Accuracy: " ,acc*100)

    
# serialize model to JSON
model_json = net1.to_json()
with open("net1.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
net1.save_weights("net1.h5")

with open("acc_net1.json", "w") as json_file:
